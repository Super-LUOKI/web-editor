...
export class RenderKeyFrameAnimation extends RenderAnimation {
  ...
  private getBothSideKeyFrameAnim(time: number, keyframes: KeyFrameAnimation['keyframes']) {
    ...
    const preKeyframeIndex = sortedKeyframes.findIndex(
      (keyframe, index) =>
        keyframe.start <= time &&
        (!sortedKeyframes[index + 1] || time < sortedKeyframes[index + 1].start)
    )
    const preKeyframe = preKeyframeIndex !== -1 ? sortedKeyframes[preKeyframeIndex] : undefined
    const nextKeyframe =
      preKeyframeIndex + 1 < sortedKeyframes.length
        ? sortedKeyframes[preKeyframeIndex + 1]
        : undefined
  ...
  getAnimationProperty(time: number) {
    ...
    if (!preKeyframe && keyframes.length > 0) {
      ...
      Object.entries(fromProps).forEach(([animAttr]) => {
        ...
        currentProps[animAttr] = interpolate(
          time - animFrom,
          [0, animTo - animFrom],
          [fromProps[animAttr], toProps[animAttr]],
          { easing: easingFn }
        )
      })
    }
