...
export interface IElementOperation<T extends AllElementType> {
  getAttribute<K extends keyof ElementOfType<T>>(
    key: K,
    placeholder?: ElementOfType<T>[K]
  ): ElementOfType<T>[K] | undefined
  update(element: Partial<Omit<ElementOfType<T>, 'id'>>): void
}

export class EditorElement<T extends AllElementType = AllElementType>
implements IElementOperation<T>
{
  constructor(
