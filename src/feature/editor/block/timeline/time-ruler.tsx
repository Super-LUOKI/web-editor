...
export function TimeRuler() {
  ...
  const maxDisplayDuration = Math.min(maxDuration, duration)

  const section = getMatchedRulerSection(pixelPerSecond)

  return (
    <div
      className='relative bg-gray-100 text-xs text-gray-800 flex items-center select-none'
      style={{
        width: `${maxDisplayDuration * pixelPerSecond}px`,
        height: 16,
      }}
    >
      {
